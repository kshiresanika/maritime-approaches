...
check("labels map onto contracts.SensorKind, no parallel enum",
      set(LABEL_TO_KIND.values()) == {"video_stream", "mac_camera", "file"})
...
sw = SourceSwitch(active="VIDEO_STREAM")
check("the active kind is accepted", sw.accepts("video_stream") is True)
...
check("a demoted node gets an explanation, not silence",
      "VIDEO_STREAM is the live source" in sw.demoted().get("mac-cam-01", ""))
...
check("the old authority is now the demoted one", sw.accepts("video_stream") is False)
...
order = ["VIDEO_STREAM", "MAC_CAMERA", "RECORDED"]
N = 3000
times: list[float] = []
for i in range(N):
...
if not _HAVE_SERVER:
    ...
    async def main() -> None:
        ...
        await b.ingest(pi, node_id="sensor-01", kind="video_stream", measured_fps=4.2)
        check("a demoted node's contacts are BUFFERED",
              len(b._last_by_kind.get("video_stream", [])) == 3)
        check("...and its liveness IS recorded (posting != selected)",
              b._nodes.get("sensor-01") is not None
              and b._nodes["sensor-01"].online is True)
        check("...and it did NOT become the picture", b._demo.mode == "replay")
        check("...and the console can explain why",
              "RECORDED is the live source" in b.source.demoted().get("sensor-01", ""))
        check("measured_fps passed through from the node", 
              b._nodes["sensor-01"].measured_fps == 4.2)
        ...
        res = await b.set_source("MAC_CAMERA", operator_id="test", reason="video source stalled")
        ...
        await b.ingest([mk_contact(9, "edge")], node_id="sensor-01",
                       kind="video_stream", measured_fps=4.0)
        ...
        check("...but it IS shown as alive again",
              b._nodes["sensor-01"].online is True)
        ...
        n = b._nodes["sensor-01"]
        b._nodes["sensor-01"] = SensorNode(
        ...
        check(f"a node silent for {stale_after + 0.5:.1f}s flips to OFFLINE",
              b._nodes["sensor-01"].online is False)
